...
class TransformerDecoderPrefix(nn.Module):
    """
    Run a prefix of the layers of a `TransformerDecoder`, and return the input
    and output embeddings of the last layer.
    ...
    Args:
        ...
        head_dim (int): embedding dimension for each head in self-attention. This is used
            to setup the :func:`~torchtune.modules.KVCache`
        target_layer (int): Index of the last layer to run, whose inputs and
    ...
    def __init__(
        ...
        self.tok_embeddings = tok_embeddings
        self.layers = _get_clones(layer, target_layer + 1)
        self.max_seq_len = max_seq_len
        self.num_heads = num_heads
        self.head_dim = head_dim
        self.causal_mask = None
    ...
    def forward(self, tokens: Tensor, input_pos: Optional[Tensor] = None) -> Tensor:
        ...
        for i, layer in enumerate(self.layers):
            ...
            if i == self.target_layer:
                last_input_h = h.clone()
            h = layer(h, mask, input_pos)

        return last_input_h, h
